...
class ModernCalendar(QWidget):
    def __init__(self):
        ...
        # Variable to store mouse press position for dragging
    ...
    def mousePressEvent(self, event):
        # Only allow dragging if the click occurs inside the HeaderBar widget
        if self.header_bar.underMouse() and event.button() == Qt.MouseButton.LeftButton:
            # Capture global mouse position minus current top-left window position
            self.drag_position = (
    ...
    def mouseMoveEvent(self, event):
        if (
            ...
            # Calculate new position and move the entire widget window
            self.move(event.globalPosition().toPoint() - self.drag_position)
            event.accept()

    def mouseReleaseEvent(self, event):
        # Reset drag tracking position on release
        self.drag_position = QPoint()
        event.accept()

    def highlight_holidays(self):
        holiday_format = QTextCharFormat()
        holiday_format.setForeground(QColor("#E81123"))
